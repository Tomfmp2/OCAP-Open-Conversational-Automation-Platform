...
public interface IVectorDatabase
{
    ...
    Task UpsertVectorsAsync(Guid tenantId, List<EmbeddingVector> vectors, CancellationToken cancellationToken = default) => UpsertVectorsAsync(tenantId, (IEnumerable<EmbeddingVector>)vectors, cancellationToken);
    Task<List<KnowledgeSearchResult>> SearchVectorsAsync(Guid tenantId, float[] queryVector, int topK = 5, double minScore = 0.5, CancellationToken cancellationToken = default);
