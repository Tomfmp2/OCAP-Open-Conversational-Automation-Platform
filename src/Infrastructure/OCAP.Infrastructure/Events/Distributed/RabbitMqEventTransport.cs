...
// Transporte distribuido de producción para RabbitMQ (Topic Exchanges, DLX/DLQ, Confirmations, PreFetch QoS) (CAP-20).
public class RabbitMqEventTransport : IEventTransport
{
    ...
    public RabbitMqEventTransport(IEventSerializer serializer, ILogger<RabbitMqEventTransport> logger)
    {
    ...
    {
        _isConnected = true;
        _logger.LogInformation("Transporte RabbitMQ conectado exitosamente.");
    ...
    {
        _isConnected = false;
        _logger.LogInformation("Transporte RabbitMQ desconectado.");
        return Task.CompletedTask;
    }
    ...
    public async Task PublishAsync<TEvent>(TEvent @event, EventEnvelope<TEvent> envelope, CancellationToken cancellationToken = default) where TEvent : IEvent
    {
        ...
        _logger.LogDebug("Publicando evento {EventType} en RabbitMQ Exchange 'ocap.events'...", typeof(TEvent).Name);
